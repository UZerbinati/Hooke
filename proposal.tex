...
A powerful framework to include such effects is \textbf{Capriz's order parameter manifold} concept [REF], which describes using the notion of a manifold and an associated Lie group action, the set of internal states of the fluid, such as orientation, polarisation and conformation.
This geometric characterization not only encodes the nature of the microstructure but also its inherent symmetries under rotations or other transformations. Historically, Capriz's idea has been used to formulate phenomenological continuum theories of ordered fluids.
...
The project is organized into four interconnected strands, described below.
...
Our theory yields a mesoscopic model for any continuum with microstructure characterized by a given order parameter manifold. Building on this foundation, the proposed research in kinetic theory will involve:

\textbf{Hydrodynamic Limits and Closure:} We will employ novel moment-closure techniques to derive macroscopic equations (analogous to Navier--Stokes or liquid crystal hydrodynamics) that faithfully capture microstructural effects. In collaboration with G.~Russo and P.~E.~Farrell, I have already devised a novel closure strategy for a simplified Boltzmann--Curtiss system, inspired by Rational Thermodynamics, which successfully reproduces the anisotropic stress of nematic liquid crystals and yields a compressible variant of the Leslie--Ericksen equations. I plan to extend this approach, working with J.~Málek, to obtain thermodynamically consistent continuum models for arbitrary microstructures. A key challenge will be ensuring that the closure respects the underlying entropy law and symmetry constraints of the kinetic theory.
    
\textbf{Algebraic and Geometric Properties of the Manifold} We plan to examine the impact of the manifold's geometry (curvature, topology) on the kinetic theory. In particular, I will investigate whether certain geometric conditions (e.g. convexity or curvature bounds on the manifold) allow a separation of scales on the manifold, introducing slowly-varying and rapidly-varying coordinates in the BBGKY derviation presented in [REF].
...
Furthermore, in [REF] we showed that if this action is transitive, then the only collision invariants of the kinetic equation are the classical microscopic invariants (mass, linear momentum, generalised angular momentum and energy). This is a profound restriction arising purely from symmetry.
I will explore further: for example, using the orbit stabiliser theorem the consequence that the symmetries of the Lie group has on the kinetic theory of fluids with internal structures.
...
To systematically compute multiple solutions, I will employ and extend \emph{deflation} methods, which have been the research focus of P.~E.~Farrell.
